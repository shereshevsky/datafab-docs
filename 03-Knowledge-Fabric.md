...
The Knowledge Fabric serves as the foundational data integration and intelligence layer for the DataFab platform. It implements a metadata-driven architecture that provides unified access to distributed data assets while leaving source data in place. The Knowledge Fabric is fundamentally an access enabler, not a data repository—it maintains mappings and relationships rather than duplicating data, ensuring a single source of truth while providing unified analytical capabilities.
...
The Knowledge Graph serves as the foundation for entity management, relationship tracking, and data integration across the DataFab platform.
...
| Asset | Data resources and items | Asset ID, type, classification |
| Document | Data files and records | Type, classification, retention |
...
| REFERENCES | Organization | Person/Organization | Reference type |
...
| Address Standardization | Location matching | Standard normalization |
...
|:---------|:---------|:-----------|
| Corporate Registries | Business registries, SEC filings | Incorporation, officers, filings |
| Identity Verification | ID document validation | Identity confirmation |
| Business Data | Business credit agencies | Company information, financials |
| Sanctions Lists | Public sanctions lists | Designated persons/entities |
| Data Providers | Third-party data providers | Professional information |
| Adverse Media | News aggregators, media monitors | News, public information |
| Court Records | Legal databases | Public litigation records |
| Public Databases | Government data sources | Public information |
...
External data extractors use schemas defined by the user to structure incoming data. This ensures external data conforms to the organization's domain model.
...
│  │                    CONNECTOR LAYER                      │    │
│  │ ┌────────┐ ┌─────────┐ ┌────────┐ ┌─────────┐ ┌───────┐ │    │
│  │ │  CRM   │ │Analytics│ │  Data  │ │ Finance │ │Custom │ │    │
│  │ │ Systems│ │ Systems │ │  Mgmt  │ │ Systems │ │ APIs  │ │    │
│  │ └────────┘ └─────────┘ └────────┘ └─────────┘ └───────┘ │    │
│  └─────────────────────────────────────────────────────────┘    │
│                               │                                 │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │                    CUSTOMER SYSTEMS                     │    │
│  │  Data Mgmt │ CRM │ Analytics │ Document Mgmt │ APIs     │    │
│  └─────────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────────┘
```
...
| :------------------ | :----------------- | :--------------------------------- |
| Data Warehouses     | API, database      | Data assets, schemas, metadata     |
| Analytics Platforms | API, database      | Dashboards, models, insights       |
| Data Lakes          | API, database      | Data catalog, lineage              |
| CRM Systems         | API                | Relationships, interactions        |
| Document Management | API                | Document metadata, classifications |
| ERP / Billing       | API, database      | Financial data, transaction records |
| Email Archives      | API                | Communication metadata, attachments |
...
| IP Allowlisting | Customer can restrict to DataFab IPs |
...
The Knowledge Fabric enables access to source systems while maintaining data governance and audit requirements.
...
| Cross-System Relationships | Relationships discovered across databases         |
...
| Data Records | Financial data in source systems |
...
| IP Allowlisting | Source system can restrict to DataFab IPs |
...
| Document Management | SharePoint, Document stores, Box                                                      |
| Data Systems        | Data warehouses, data lakes, analytics platforms                                      |
...
DataFab maintains MCP connectors through proactive monitoring, continuous health tracking, and defined remediation processes to ensure service continuity as third-party platforms evolve.
...
|:---------------|:------|:------------|
| Issue Detection | DataFab | Automated monitoring identifies connector issues |
| Root Cause Analysis | DataFab | Determine if vendor change, configuration, or platform issue |
| Connector Updates | DataFab | Code updates, compatibility fixes, regression testing |
| Customer Notification | DataFab | Proactive communication of detected issues and remediation timeline |
...
| Sensitive | Confidential business data | Enhanced access controls |
...
|:-----------------|:------------|:--------|
| Entity Types (E_types) | Allowed entity categories | Person, Organization, Asset |
| Relation Types (R_types) | Allowed relationship types | OWNS, REPRESENTS, REFERENCES |
| Attribute Types (A_types) | Allowed entity attributes | Name, Date, Amount, Classification |
...
| Restricted | PII, privileged data | Enhanced encryption, masking |
| Highly Restricted | Credentials, privileged data | HSM encryption, privileged access |
